    ...
    <div :class="{ rtl: localeStore.isRTL }">
      <div class="flex h-screen overflow-hidden bg-brand-systemBg select-none">
        ...
        >
          <!-- Logo -->
          <div
            class="flex items-center border-b border-[var(--border-subtle)] h-14 px-3 flex-shrink-0 overflow-hidden"
            :class="isCollapsed ? 'justify-center' : 'gap-3'"
          >
            <div
              class="w-8 h-8 bg-brand-primary rounded-lg flex items-center justify-center flex-shrink-0"
            >
            ...
            >
              <span
                class="text-tx-primary text-[13px] font-bold leading-tight truncate"
                >Nexux ERP</span
              >
              <span
                class="text-[10px] tracking-widest text-tx-muted font-medium truncate"
              >
          ...
          >
            <template v-if="authStore.isAuthenticated">
              ...
              <lazy-accountant-layout
                v-if="isAccountant"
                :collapsed="isCollapsed"
              />

              <lazy-employee-layout
                v-if="isEmployee"
                :collapsed="isCollapsed"
              />
              ...
              <lazy-system-auditor-layout
                v-if="isSystemAuditor"
                :collapsed="isCollapsed"
              />

              <lazy-inventory-manager-layout
                v-if="isInventoryManager"
                :collapsed="isCollapsed"
              />

              <lazy-td-officer-layout
                v-if="isTdOfficer"
                :collapsed="isCollapsed"
              />

              <lazy-maintenance-technician-layout
                v-if="isMaintenanceTechnician"
                :collapsed="isCollapsed"
              />
            </template>
          </nav>
          ...
          <div
            class="border-b border-[var(--border-subtle)] p-3 flex-shrink-0 overflow-hidden"
          >
            <!-- User info -->
            <div
              v-if="!isCollapsed"
            ...
            >
              <p
                class="text-xs font-semibold text-tx-primary truncate leading-tight"
              >
                {{
              ...
              <p
                v-if="currentRole"
                class="text-[10px] text-tx-muted truncate leading-tight mt-0.5"
              >
                {{ t("roles." + currentRole) }}
              </p>
            </div>
            ...
            <LazyVButton
              ...
              variant="ghost"
              :block="true"
              text-color="text-tx-secondary"
              hover-color="hover:bg-[var(--bg-hover)]"
              padding-x="px-2"
              :padding-y="isCollapsed ? 'py-2' : 'py-1.5'"
              class="rounded-lg hover:text-tx-primary text-xs justify-start p-2"
              @click="handleLogout"
            >
              <Icon
              ...
              />
              <span v-if="!isCollapsed" class="truncate ms-2">{{
                t("btn.logout")
              }}</span>
            </LazyVButton>
          </div>
        ...
        <transition name="slide-sidebar">
          ...
          >
            <div
              class="flex items-center gap-3 border-b border-[var(--border-subtle)] h-14 px-4 flex-shrink-0"
            >
              <div
                class="w-8 h-8 bg-brand-primary rounded-lg flex items-center justify-center flex-shrink-0"
              >
              ...
              <div class="flex flex-col min-w-0 flex-1">
                <span
                  class="text-tx-primary text-[13px] font-bold leading-tight"
                  >Nexux ERP</span
                >
                <span
                  class="text-[10px] tracking-widest text-tx-muted font-medium"
                >
                  {{ currentRole ? t("roles." + currentRole) : "ERP" }}
                </span>
              </div>
            ...
            <nav
              class="flex-1 overflow-y-auto hide-scrollbar py-3 px-2 space-y-0.5"
            >
              <template v-if="authStore.isAuthenticated">
            ...
            <div
              class="border-b border-[var(--border-subtle)] p-3 flex-shrink-0"
            >
              <!-- User info -->
              <div
                class="mb-3 px-1 bg-[var(--bg-hover)] hover:bg-[var(--bg-hover-strong)] transition-colors cursor-pointer rounded-lg py-2 text-center"
              ...
              <LazyVButton
                ...
                padding-x="px-2"
                padding-y="py-1.5"
                class="rounded-lg hover:text-tx-primary text-xs gap-2 justify-start"
                @click="handleLogout"
              >
                <Icon
                  name="material-symbols:logout-rounded"
        ...
        <div class="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div class="flex items-center gap-2.5 sm:gap-3 py-1">
                ...
                <div
                  :class="[
                    'flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr shadow-sm transition-all duration-300 hover:scale-105',
                    greeting.bgClass,
                  ]"
                >
                  <Icon
                    :name="greeting.icon"
                    :class="[
                      'w-6 h-6 sm:w-8 sm:h-8 drop-shadow-sm',
                      greeting.colorClass,
                    ]"
                  />
                </div>
                ...
                <div class="flex flex-col justify-center min-w-0">
                  ...
                  >
                    ...
                    <span
                      v-if="authStore.currentUser?.user_metadata?.fullName"
                      class="text-tx-secondary font-normal text-[10px] sm:text-xs"
                    >
                      ,
                      {{
                        authStore.currentUser.user_metadata.fullName.split(
                          " ",
                        )[0]
                      }}
                    </span>
                  ...
                  >
                    ...
                    <span
                      class="text-[var(--border-subtle)] font-bold select-none"
                      >•</span
                    >
                    <!-- Monospace Animated clock -->
                    <div
                      class="flex items-center text-tx-secondary font-mono font-semibold tracking-wider"
                    >
                      <span
                        v-for="(char, idx) in timeChars"
                        :key="idx"
            ...
            <div class="flex items-center gap-2">
              ...
              <LazyVButton
                ...
                padding-x="px-1.5"
                padding-y="py-1.5"
                class="lg:hidden rounded-lg hover:text-tx-primary"
          ...
          <div
            class="flex min-h-0 flex-1 flex-col overflow-y-auto px-4 sm:px-6 py-4 hide-scrollbar"
          >
            <slot />
          </div>
        </div>
...
const greeting = computed(() => {
  ...
  if (hour >= 5 && hour < 12) {
    return {
      ...
      bgClass:
        "from-amber-500/5 to-amber-500/15 border-amber-500/20 dark:from-amber-500/10 dark:to-amber-500/25 dark:border-amber-500/30",
    };
    ...
    return {
      ...
      bgClass:
        "from-orange-500/5 to-orange-500/15 border-orange-500/20 dark:from-orange-500/10 dark:to-orange-500/25 dark:border-orange-500/30",
    };
    ...
    return {
      ...
      bgClass:
        "from-indigo-500/5 to-indigo-500/15 border-indigo-500/20 dark:from-indigo-500/10 dark:to-indigo-500/25 dark:border-indigo-500/30",
    };
  } else {
    return {
      ...
      bgClass:
        "from-blue-500/5 to-blue-500/15 border-blue-500/20 dark:from-blue-500/10 dark:to-blue-500/25 dark:border-blue-500/30",
    };
  }
});
...
const formattedDate = computed(() => {
  return new Intl.DateTimeFormat(
    localeStore.locale === "ar" ? "ar-EG" : "en-US",
    {
      weekday: "short",
      month: "short",
      day: "numeric",
