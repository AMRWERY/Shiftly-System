...
const getFormattedCellValue = (item: any, column: Column<any>, index: number): string => {
  ...
  if (typeof column.key !== 'string') {
    return item && column.key in item ? String(item[column.key as keyof typeof item] ?? '') : '';
  }
  const keys = column.key.split('.');
  let value = item;
  for (const k of keys) {
...
const exportToPdf = (headers: string[], dataRows: string[][], fileName: string): boolean => {
...
const downloadData = (format: 'pdf' | 'excel') => {
  ...
  const dataRows = allItemsToExport.value.map((item, index) =>
    columnsToExport.map(col => stripHtml(getFormattedCellValue(item, col, index)))
  );
  ...
  try {
    ...
    else if (format === 'excel') success = exportToExcel(headers, dataRows, fileName);
