<template>
    <div class="flex min-h-0 flex-1 flex-col">
        <!-- MAIN CARD -->
        <div class="rounded-2xl border border-white/5 bg-brand-layoutBg shadow-xl overflow-hidden">
            <!-- TABS -->
            <div class="flex items-center gap-8 px-6 border-b border-white/5 bg-black/10">
                <button
            ...
            <div class="flex-1">
                <ul class="flex flex-col">
                    <li v-for="(item, index) in mockNotifications" :key="index"
                        ...
                        <div class="flex-shrink-0 mt-0.5 flex h-12 w-12 items-center justify-center rounded-xl"
                            :class="item.iconBg">
                            <Icon :name="item.icon" class="w-6 h-6" :class="item.iconColor" />
                        </div>
                        <div class="flex-grow">
                            <div class="flex items-center justify-between mb-1.5">
                                <p class="text-base font-medium text-gray-200">{{ item.title }}</p>
                                <span class="text-xs font-medium tracking-wider text-gray-500 uppercase">{{ item.time
                                    }}</span>
                            </div>
