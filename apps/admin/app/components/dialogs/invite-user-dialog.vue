<template>
  <div>
    <VDialog :model-value="isOpen" :title="t('dialog.invite_user_title')" :show-default-footer="false"
          ...
          <form @submit.prevent="inviteUser">
            <div class="space-y-4 overflow-y-auto max-h-[350px] hide-scrollbar">
              ...
              <div class="grid grid-cols-1 gap-4">
                <div>
                  <VInput :label="t('form.first_name')" :name="t('form.first_name')" type="text"
                    rules="required" :required="true" v-model="form.firstName" />
                </div>

                <div>
                  <VInput :label="t('form.middle_name')" :name="t('form.middle_name')" type="text"
                    v-model="form.middleName" />
                </div>

                <div>
                  <VInput :label="t('form.last_name')" :name="t('form.last_name')" type="text" :required="true"
                    rules="required" v-model="form.lastName" />
                </div>

                <!-- Email -->
                <div>
                  <VInput :label="t('form.email')" :name="t('form.email')" type="email" :required="true"
                    rules="required|email" v-model="form.email" />
                </div>

                <!-- Phone Number -->
                <div>
                  <VInput :label="t('form.phone_number')" :name="t('form.phone_number')" type="tel"
                    v-model="form.phoneNumber" />
                </div>

                <!-- Role -->
                <div>
                  <VInput :label="t('form.role')" :name="t('form.role')" type="select" :required="true"
                    rules="required" :placeholder="t('form.select_role')" :options="roles" v-model="form.role" />
                </div>

                <!-- Base Salary -->
                <div>
                  <VInput :label="t('form.base_salary')" :name="t('form.base_salary')" type="number"
            ...
            <div class="mt-6 flex justify-end gap-3">
              <VButton type="button" variant="outline" :border-color="'border-gray-700'" :text-color="'text-white'"
                padding-x="px-4" padding-y="py-2" @click="closeModal" class="hover:bg-gray-700 transition-colors">
                {{ t('btn.cancel') }}
              </VButton>
              <VButton type="submit" :disabled="loading" hover-color="hover:bg-blue-700" padding-x="px-4"
                padding-y="py-2"
