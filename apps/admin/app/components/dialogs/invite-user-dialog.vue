    ...
    <LazyVDialog :model-value="isOpen" :title="t('dialog.invite_user_title')" :show-default-footer="false"
      ...
      <template #body>
        <div class="mt-2">
          ...
          <LazyVFormWrapper @submit="inviteUser">
            <div class="space-y-4 overflow-y-auto max-h-[350px] hide-scrollbar">
              <!-- Name Fields -->
              <div class="grid grid-cols-1 gap-4">
                <div>
                  <LazyVInput :label="t('form.first_name')" :name="t('form.first_name')" type="text" rules="required"
                    :required="true" v-model="form.firstName" />
                </div>
                ...
                <div>
                  <LazyVInput :label="t('form.last_name')" :name="t('form.last_name')" type="text" :required="true"
                    rules="required" v-model="form.lastName" />
                </div>

                <!-- Email -->
                <div>
                  <LazyVInput :label="t('form.email')" :name="t('form.email')" type="email" :required="true"
                    rules="required|email" v-model="form.email" />
                </div>

                <!-- Phone Number -->
                <div>
                  <LazyVInput :label="t('form.phone_number')" :name="t('form.phone_number')" type="tel"
                    v-model="form.phoneNumber" />
                </div>

                <!-- Role -->
                <div>
                  <LazyVInput :label="t('form.role')" :name="t('form.role')" type="select" :required="true"
                    rules="required" :placeholder="t('form.select_role')" :options="roles" v-model="form.role" />
                </div>

                <!-- Base Salary -->
                <div>
                  <LazyVInput :label="t('form.base_salary')" :name="t('form.base_salary')" type="number"
            ...
            <div class="mt-6 flex justify-end gap-3">
              <LazyVButton type="button" variant="outline" :border-color="'border-[var(--border-default)]'" :text-color="'text-tx-primary'"
