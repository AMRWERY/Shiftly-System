...
const submitEdit = async () => {
    ...
    loading.value = true
    try {
        // @ts-ignore - The types are generated by Nuxt and might not be fully up-to-date in the IDE
        await usersStore.updateUser(props.user.id, form.value)
        triggerToast({
            message: t('toast.user_updated_successfully'),
            type: 'success',
        ...
        triggerToast({
            message: error.statusMessage || error.message || t('toast.failed_to_update_user'),
            type: 'error',
