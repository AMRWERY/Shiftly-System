<template>
  <div :dir="locale === 'ar' ? 'rtl' : 'ltr'">
    <label :for="id"
      :class="labelClass || ['block mb-1 text-sm font-medium', errorMessage ? 'text-red-500 dark:text-red-300' : 'text-tx-secondary']">
      {{ label }}
    ...
    <div class="relative w-full">
      ...
      <span v-if="prefixIcon"
        :class="['absolute inset-y-0 flex items-center text-gray-400 start-3 hover:text-white transition-colors', errorMessage ? 'text-red-500 dark:text-red-300' : 'text-tx-muted']">
      ...
      <template v-if="type === 'textarea'">
        <Field as="textarea" :name="name" :placeholder="placeholder" :id="id" :readonly="readonly" :options="options"
          ...
          <textarea :name="name" :placeholder="placeholder" :id="id" :readonly="readonly" v-model="internalValue"
            :class="[
              'w-full bg-bg-primary border rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50 transition-all text-sm placeholder:text-tx-muted text-tx-primary',
              prefixIcon ? 'ps-11' : 'ps-4',
              fieldError ? 'border-red-500/50' : 'border-[var(--border-default)]'
            ]" rows="4" />
        </Field>
      </template>
      ...
      <template v-else-if="type === 'select'">
        <Field as="select" :name="name" :placeholder="placeholder" :id="id" :readonly="readonly" v-model="internalValue"
          :rules="rules" :class="[
            'w-full bg-bg-primary border rounded-lg py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50 transition-all text-sm text-tx-primary',
            prefixIcon ? 'ps-11 pe-4' : 'px-4',
            'border-[var(--border-default)]'
          ]">
          <option value="" disabled selected class="text-tx-muted bg-bg-primary">{{ placeholder }}</option>
          <option v-for="option in options" :key="option.value" :value="option.value" class="text-tx-primary bg-bg-primary">
            {{ option.label }}
          </option>
        </Field>
      </template>
        ...
        <Field :type="showPassword ? 'text' : type" :name="name" :placeholder="placeholder" :id="id" :options="options"
          ...
          <input :type="showPassword ? 'text' : type" :name="name" :placeholder="placeholder" :id="id"
            :readonly="readonly" v-model="internalValue" :class="[
              'w-full bg-bg-primary border rounded-lg py-3 pe-4 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50 transition-all text-sm placeholder:text-tx-muted text-tx-primary',
              prefixIcon ? 'ps-11' : 'ps-4',
              fieldError ? 'border-red-500/50' : 'border-[var(--border-default)]'
            ]" />
        </Field>
    ...
    <!-- error messages -->
    <LazyErrorMessage :name="name" class="block mt-1 text-[13px] text-red-500 dark:text-red-300" />
  </div>
</template>
