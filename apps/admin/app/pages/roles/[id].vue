<template>
    <div>
        <div class="p-6">
            ...
            <div v-if="role" class="max-w-6xl mx-auto animate-fade-in">
                ...
                <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8 mb-6">
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                        <div class="flex-1">
                            ...
                            <code class="px-2 py-1 bg-gray-100 text-gray-800 rounded text-sm">{{ role.name }}</code>
                            <p v-if="role.description" class="mt-4 text-gray-600">{{ role.description }}</p>
                        </div>

                        <div class="flex gap-3">
                            <base-button v-if="!role.isSystemRole" type="button" variant="outline"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-200">
                        <div class="flex items-center gap-3 p-4 bg-indigo-50 rounded-lg">
                            <div class="p-3 bg-indigo-100 rounded-lg">
                                <icon name="heroicons:users" class="w-6 h-6 text-indigo-600" />
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">{{ t('roles.total_users') }}</p>
                                <p class="text-2xl font-bold text-gray-900">{{ role.userCount || 0 }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 p-4 bg-green-50 rounded-lg">
                            ...
                            <div>
                                <p class="text-sm text-gray-600">{{ t('roles.permissions_granted') }}</p>
                                <p class="text-2xl font-bold text-gray-900">{{ permissionsCount }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 p-4 bg-purple-50 rounded-lg">
                            ...
                            <div>
                                <p class="text-sm text-gray-600">{{ t('roles.created_on') }}</p>
                                <p class="text-lg font-semibold text-gray-900">{{ formatDate(role.createdAt) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-6">
                    <h2 class="text-xl font-semibold text-gray-900 mb-6">{{ t('roles.role_permissions') }}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div v-for="user in users" :key="user.id"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-medium text-gray-900 truncate">{{ user.first_name }} {{ user.last_name }}
                                </p>
...
const permissionsCount = computed(() => {
    ...
    return role.value.permissions.reduce((sum, perm) => sum + perm.actions.length, 0)
})
...
useHead({
    title: computed(() =>
        role.value
            ? `${role.value.displayName} | ${t('roles.role_details')}`
            : t('meta.roles')
    ),
})
